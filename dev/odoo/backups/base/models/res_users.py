    ...
    def init(self):
        ...
        if self.env.cr.rowcount:
            ...
            for uid, pw in cr.fetchall():
                Users.browse(uid).franchise_password = pw

    def _set_password(self):
        ctx = self._crypt_context()
        for user in self:
            self._set_encrypted_password(user.id, ctx.hash(user.franchise_password))
    ...
    def _set_new_password(self):
        for user in self:
            ...
            if user == self.env.user:
                ...
                user.franchise_password = user.new_password
